...
def table_model_init(table_model_type, model_path, max_time, _device_="cpu"):
    if table_model_type == MODEL_NAME.STRUCT_EQTABLE:
        ...
        config = {"model_dir": model_path, "device": _device_}
...
def mfr_model_init(weight_dir, cfg_path, _device_="cpu"):
    ...
    mfr_transform = transforms.Compose(
        [
            vis_processor,
        ]
    )
...
def atom_model_init(model_name: str, **kwargs):
    ...
    if model_name == AtomicModel.Layout:
        ...
        atom_model = mfd_model_init(kwargs.get("mfd_weights"))
...
class CustomPEKModel:
    ...
    def __call__(self, image):
        ...
        if self.layout_model_name == MODEL_NAME.LAYOUTLMv3:
            ...
            # doclayout_yolo
            layout_res = []
            doclayout_yolo_res = self.layout_model.predict(
                image, imgsz=1024, conf=0.25, iou=0.55, verbose=True, device=self.device
            )[0]
        ...
        if self.apply_table:
            ...
            for res in table_res_list:
                ...
                if latex_code:
                    expected_ending = latex_code.strip().endswith(
                        "end{tabular}"
                    ) or latex_code.strip().endswith("end{table}")
                    if expected_ending:
                        res["latex"] = latex_code
                    else:
                        logger.warning(
                            f"table recognition processing fails, not found expected LaTeX table end"
                        )
                elif html_code:
                    res["html"] = html_code
                else:
                    logger.warning(
                        f"table recognition processing fails, not get latex or html return"
                    )
